...
type WorkspaceCanvasElement = globalThis.HTMLCanvasElement & {
  requestPaint?: () => void;
  onpaint?: (() => void) | null;
};
...
interface WorkspaceState {
  ...
  didPan: boolean;
  viewAnimationId: number | null;
  dpr: number;
  apiMode: "fallback" | "html-in-canvas";
...
function hydrateEntities(entities: unknown[]) {
...
function hydrateEntity(input: unknown): Entity | null {
  if (!isRecord(input)) {
    return null;
  }
