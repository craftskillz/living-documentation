...
import { loadI18n } from '../i18n.svelte';
import { NODE_COLORS, NODE_L_RATIOS, DEFAULT_NODE_PALETTE, DEFAULT_EDGE_PALETTE, deriveNodeColors } from './constants.js';
import { loadCustomShapeLibraries, renderCustomShapeBar } from './custom-shapes.js';
...
function toggleSidebar() {
  ...
  const saved = parseInt(localStorage.getItem('ld-diagram-sidebar-w') || '', 10);
  const openWidth = Number.isFinite(saved) ? saved + 'px' : '14rem';
  sb.style.width    = st.sidebarOpen ? openWidth : '0';
...
// All side-effecting wiring (DOM listeners, keyboard/paste handlers, bootstrap)
// runs here so the Svelte route can call it after mount and tear it down on
// unmount. Returns a cleanup function that removes document/window-level
// listeners (element-level listeners die with the destroyed DOM).

export function initDiagram() {
  // ── setTool bridge (window-level) ──────────────────────────────────────────
  const onSetTool = (e) => {
    const detail = e.detail || {};
    if (!detail.tool) return;
    setTool(detail.tool, detail.shape);
  };
  window.addEventListener('diagram:setTool', onSetTool);
  ...
  // ── Bootstrap (config, palettes, defaults, custom shapes, diagram list) ──────
  (async () => {
    ...
    } catch { await loadI18n('en'); /* config unavailable — use defaults */ }
  ...
  // ── Cleanup ─────────────────────────────────────────────────────────────────
  return () => {
    window.removeEventListener('diagram:setTool', onSetTool);
