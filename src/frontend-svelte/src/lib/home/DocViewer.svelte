    ...
    <div class="flex items-start gap-4 flex-wrap">
      <div class="shrink min-w-0">
        ...
        {#if doc.tags && doc.tags.length}
          <div class="mt-1.5 flex items-center gap-1.5 flex-wrap" data-testid="doc-tags">
            {#each doc.tags as tag (tag)}
              <span class="inline-block text-[11px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700">#{tag}</span>
            {/each}
          </div>
        {/if}
      </div>
