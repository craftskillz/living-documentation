...
{#if report && report.total > 0}
  ...
  <div onclick={onopen} title={tooltip} class="no-print mt-5 flex items-center gap-3 mr-auto w-full sm:w-80 cursor-pointer select-none">
    ...
    <span class="text-xs font-semibold shrink-0 tabular-nums" style="color:{accuracyColor(report.accuracy)}">{pct}%</span>
  </div>
{/if}
