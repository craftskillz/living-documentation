...
**tags:** mcp, server-guide, adr, feature-workflow, supersede, update_document, get_server_guide, frontmatter, living-documentation, markdown-conventions
---
...
- Aucun moyen n'existait pour *modifier* un document : `create_document` refuse les collisions de nom. Impossible donc de « superseder » une ADR obsolète sans intervention manuelle dans le filesystem.
...
Le guide stipule qu'une feature n'est *terminée* qu'après la création (ou supersede) d'une ADR liée aux fichiers sources concernés. Étapes obligatoires :
...
- Le guide est long (~280 lignes). Les clients MCP qui *résument* `instructions` au lieu de le coller verbatim peuvent perdre des règles. Atténué par l'outil `get_server_guide` redondant.
