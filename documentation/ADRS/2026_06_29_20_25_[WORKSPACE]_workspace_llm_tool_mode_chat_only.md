...
**description:** Les providers LLM du Workspace persistent un toolMode herite par leurs agents pour autoriser les payloads tools MCP ou forcer un mode chat-only explicite.
**tags:** workspace, llm-provider, toolMode, chat-only, mcp-tools, runAgent, CHAT_ONLY_TOOL_NOTICE, tokenValueForPersistence, Workspace.svelte, workspace.ts, provider-compatibility
---
