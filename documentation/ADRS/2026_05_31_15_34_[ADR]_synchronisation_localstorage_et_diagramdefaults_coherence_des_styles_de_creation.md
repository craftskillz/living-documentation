...
```js
localStorage.setItem('ld-node-style-' + shapeType, JSON.stringify({
  colorKey, fontSize, width, height, textAlign: null, textValign: null,
}));
```
...
localStorage.setItem('ld-free-arrow-style', JSON.stringify({
  arrowDir, dashes, fontSize, edgeColor: null, edgeWidth: null,
}));
```
