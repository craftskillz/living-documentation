...
A checkbox "Treat every newline as a line break" in the _Appearance & Metadata_ card of `src/frontend/admin.html`, placed just below the _Code block max height_ field. Bound to `#field-soft-breaks`. Loaded in `loadConfig()` via `cfg.markdownSoftBreaks`, written in `saveConfig()` inside the existing PUT payload.
