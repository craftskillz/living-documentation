...
2. Lire ce fichier : `documentation/AI/PROJECT-INSTRUCTIONS.md`.
3. Lire `documentation/AI/PROJECT-STACK.md` pour comprendre la stack, l'arborescence utile, les concepts centraux et les conventions structurantes.
4. Lire `documentation/AI/PROJECT-USEFUL-COMMANDS.md` pour connaître les commandes de développement, build, test, lint et setup.
...
Le dossier `documentation/WORKLOG/` contient l'état opérationnel des tâches en cours et les points de reprise entre assistants IA. Il ne remplace pas les ADR.
Le dossier `documentation/WORKLOG/` contient trois types de fichiers complémentaires, aucun d'entre eux ne remplace les ADR durables qui vivent dans `documentation/ADRS/`.

### 1. `documentation/WORKLOG/current-task.md` , point de reprise partagé
...
### 2. `documentation/WORKLOG/ROADMAP.md` , backlog de tickets
...
2. crée un document dédié dans `documentation/WORKLOG/` qui consigne la réalisation (voir point 3).
...
### 3. `documentation/WORKLOG/YYYY_MM_DD_HH_mm_[WORKLOG]_ticket_XX_<slug>.md` , trace de réalisation par ticket

À chaque fois qu'un ticket de roadmap est terminé, créer un document dédié dans `documentation/WORKLOG/`. Le format ressemble à un ADR (même frontmatter) mais la **sémantique est différente** :

- les ADR (`documentation/ADRS/`) documentent les **décisions durables** du projet : architecture, contrats publics, conventions structurantes. Ils décrivent le **QUOI** et le **POURQUOI**.
...
Si un choix durable est pris pendant la réalisation d'un ticket (par exemple, le choix d'une convention de structure de dossiers), créer un ADR architectural dans `documentation/ADRS/` et faire pointer le document WORKLOG vers cet ADR , ne pas dupliquer le raisonnement.
...
`documentation/AI/PROJECT-STACK.md` contient le résumé opérationnel du projet : stack technique, zones source importantes, concepts centraux et conventions structurantes.

`documentation/AI/PROJECT-USEFUL-COMMANDS.md` contient les commandes réellement utiles : installation, développement local, build, tests, lint, formatage, setup et commandes dangereuses.
...
Les règles vivent dans `documentation/AI/rules/*.md`.
