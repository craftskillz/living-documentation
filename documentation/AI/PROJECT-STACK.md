...
## Points de vigilance issus des anciens fichiers `zozo.md` / `zaza.md`

Les anciens fichiers mélangeaient contexte projet, commandes, règles et notes profondes. La connaissance est portable, mais elle doit être répartie :

- architecture, modules, concepts et conventions générales ici ;
- commandes, setup, tests et coverage dans `PROJECT-USEFUL-COMMANDS.md` ;
- contraintes applicables par glob dans `AI/rules/*.md` ;
- détails très volumineux ou historiques dans les ADR pertinents plutôt que dans les points d'entrée agent ;
- instructions propres à un outil externe non disponible partout, comme `context7 MCP`, à garder comme recommandation conditionnelle plutôt que règle projet obligatoire.
