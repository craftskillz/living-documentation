...
- [ ] **T06** — Garde de migration au démarrage (`cli.ts`) + champ `okfMigration` en config. **PROCHAIN.**
  - Cadrage : `src/lib/config.ts` (champ `okfMigration?: { version; migratedAt }` + validation `routes/config.ts`), `bin/cli.ts` (au démarrage : lire le flag ; si absent → refuser d'ouvrir + message OKF + commande `migrate` / prompt). Nouveau projet initialisé (starters, T15) → flag déjà positionné. ADR d'implémentation (garde = contrat de démarrage) → **demander commit** (script T05 + code T06) avant bindings.
