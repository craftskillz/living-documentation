...
- [ ] **T09** — Génération des `index.md` (progressive disclosure) + `okf_version` racine. **PROCHAIN.** (`src/lib/okf/index-generator.ts` : un `index.md` par dossier au format `* [Titre](url) - description` + racine avec `okf_version: "0.1"` ; traiter `index.md`/`log.md` comme réservés dans le listing serveur.)
