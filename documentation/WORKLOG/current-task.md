...
- [ ] **T12** — Validateur de conformance OKF + hook CI. **PROCHAIN.** (Valider qu'un bundle est OKF-conforme : frontmatter YAML, `type` connu, `title` présent, `timestamp` ISO, réservés `index.md`/`log.md` + `okf_version` racine, liens bundle-relatifs résolus. Exposer en CLI `validate` + hook CI ; réutiliser `parseFrontmatter`/`deriveType`/`isReservedOkfFile`.)

## Code à committer (T11)
`src/lib/okf.ts`, `src/lib/metadata.ts`, `src/lib/migrate.ts`, `tests/unit/okf.test.ts`. Commit avant l'ADR T11 + les rebaselines T04/T06.
