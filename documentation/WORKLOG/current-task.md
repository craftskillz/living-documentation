...
- [~] **T05** — Moulinette de migration déterministe. **EN COURS.** Script `scripts/migrate-frontmatter-to-okf.ts` qui réutilise `normalizeFrontmatter` sur tous les `documentation/**/*.md` (title depuis le nom de fichier via `parseFilename`), `--dry-run`, rapport, + écrit le flag OKF dans `.living-doc.json`. Exclure plus tard `index.md`/`log.md` (T08/T09, inexistants pour l'instant).
