...
- [ ] **T10** — Génération du `log.md` depuis Git. **PROCHAIN.** (`src/lib/okf/log-generator.ts` : changelog OKF `## AAAA-MM-JJ` depuis l'historique Git du dossier docs ; appelé par `migrateDocsFolder` comme le générateur d'index.)

## Code à committer (T09)
`src/lib/okf.ts`, `src/lib/migrate.ts`, `src/lib/okf/index-generator.ts`, `src/routes/documents.ts`, `src/mcp/tools/documents.ts` (+ 31 `index.md` auto-committés). Commit avant l'ADR T09 + le rebaseline T04.
