...
- [ ] **T08** — Liens bundle-relatifs ↔ `?doc=`. **PROCHAIN.** (`export.ts` réécrit déjà `?doc=`→`.md` ; ajouter la résolution inverse dans le viewer + émettre du bundle-relatif.)

## Code à committer (T07)
`src/routes/documents.ts`, `src/frontend-svelte/src/lib/home/types.ts`, `src/frontend-svelte/src/lib/home/DocViewer.svelte`. Pas de binding ADR nouveau ; juste rebaseliner l'ADR T04 après.
