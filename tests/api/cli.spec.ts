...
function writeExistingDocsProject(docsPath: string, title: string) {
  fs.copyFileSync(
    path.resolve(__dirname, "../../starter-doc/.living-doc.json"),
    path.join(docsPath, ".living-doc.json"),
  );
  fs.writeFileSync(
    path.join(docsPath, `2026_01_01_10_00_[General]_${title}.md`),
    `# ${title}\n`,
  );
}
...
test("CLI exits 1 when the given path is a file, not a directory", () => {
  const tmp = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-")),
  );
...
test("CLI prints the listen error when the port is already in use", async () => {
  const tmp = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-port-")),
  );
  fs.mkdirSync(path.join(tmp, "docs"));
...
test("CLI with an existing docs folder keeps the server process alive", async ({
  request,
}) => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-existing-")),
  );
...
test("CLI with an explicit folder without config initializes that folder", async ({
  request,
}) => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-explicit-init-")),
  );
  ...
  try {
    ...
    expect(config.hiddenHeaderMenus).toEqual([
      "/blueprint",
      "/graph",
      "/survival-kit",
      "/context",
...
test("CLI without folder can launch existing docs from the current folder after confirmation", async ({
  request,
}) => {
  const docsRoot = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-current-")),
  );
...
test("CLI without folder can launch existing docs from a direct child folder after confirmation", async ({
  request,
}) => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-child-")),
  );
...
test("CLI without folder scaffolds starter-doc and boots the server", async () => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-init-")),
  );
  ...
  try {
    ...
    const config = JSON.parse(
      fs.readFileSync(path.join(dirAbs, ".living-doc.json"), "utf-8"),
    );
    ...
    const projectInstructions = fs.readFileSync(
      path.join(dirAbs, "AI", "PROJECT-INSTRUCTIONS.md"),
      "utf-8",
    );
    ...
    expectSymlinkTo(
      path.join(dirAbs, "AI", "AGENTS.md"),
      path.join(parent, "AGENTS.md"),
    );
    expectSymlinkTo(
      path.join(dirAbs, "AI", "CLAUDE.md"),
      path.join(parent, "CLAUDE.md"),
    );
    expectSymlinkTo(
      path.join(dirAbs, "AI", "MEMORY.md"),
      path.join(parent, "memory", "MEMORY.md"),
    );
  } finally {
    await killLD(proc);
    fs.rmSync(parent, { recursive: true, force: true });
...
test("CLI without folder and --starter-language fr scaffolds the French starter", async () => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-init-fr-")),
  );
  const dirAbs = path.join(parent, "freshdocs");
  fs.mkdirSync(dirAbs);
  const port = await pickFreePort();
  ...
  try {
    ...
    const howTo = fs.readFileSync(
      path.join(dirAbs, "AI", "2026_01_01_how_to.md"),
      "utf-8",
    );
    expect(config.language).toBe("fr");
    expect(config.hiddenHeaderMenus).toEqual([
...
test("CLI without folder creates a missing target folder", async () => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-init-missing-")),
  );
  const dirAbs = path.join(parent, "freshdocs");
  const port = await pickFreePort();
  const proc = await spawnLD({
...
test("CLI without folder replaces empty root files with starter copies and AI symlinks", async () => {
  const parent = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "ld-cli-init-empty-links-")),
  );
  const dirAbs = path.join(parent, "freshdocs");
  ...
  try {
    ...
    expectRegularFile(path.join(parent, "CLAUDE.md"));
    expectRegularFile(path.join(parent, "memory", "MEMORY.md"));
    expect(fs.readFileSync(path.join(parent, "AGENTS.md"), "utf-8")).toContain(
      "AGENTS.md",
    );
    expect(fs.readFileSync(path.join(parent, "CLAUDE.md"), "utf-8")).toContain(
      "CLAUDE.md",
    );
    expect(
      fs.readFileSync(path.join(parent, "memory", "MEMORY.md"), "utf-8"),
